...
| 5 | **Memory** | Session memory, long-term memory | ✅ Done | Conversation history in `RAGChatbot`, cached data in `.cache/`, `@st.cache_data` | Session memory: `RAGChatbot` class maintains `self.history` list storing all user/assistant message pairs for context-aware follow-up questions within a session. Long-term memory: Market data metadata persisted in `.cache/market_data_meta.json` tracks last refresh timestamps and record counts across sessions. Streamlit's `@st.cache_data(ttl=300)` provides 5-minute in-memory cache for expensive computations (stock data, ARIMA forecasts). FAISS index persists learned knowledge permanently on disk. |
